...
*This analysis is meant to mimic the analysis in the paper ["Decreasing Trend of Bone Mineral Density in US Multiethnic Population: Analysis of Continuous NHANES 2005 - 2014"](https://pubmed.ncbi.nlm.nih.gov/30091065/).* 
...
In this analysis, we apply **selective inference** using the `lmFScreen` package to compare femur neck bone mineral density (BMD) across NHANES cycles for men over age 30. Selective inference methods adjust for the fact that inference is conditioned on passing a model selection step — in our case, an overall F-test.
...
- Projects out age and intercept

- Runs `lmFScreen.fit` on the resulting matrix
...
do_year_comparison <- function(X, test_col, seed_val) {
  ...
  project_out <- cbind(1, bmd_men$Age)
...
do_year_comparison(X=cbind(bmd_men$Year_2007_2008+bmd_men$Year_2009_2010,
                           bmd_men$Year_2009_2010, bmd_men$Year_2013_2014),
                   test_col = 2, seed_val = 78910)

# Compare 2007-2008 vs 2013-2014
do_year_comparison(X=cbind(bmd_men$Year_2007_2008+bmd_men$Year_2013_2014,
                           bmd_men$Year_2009_2010, bmd_men$Year_2013_2014),
                   test_col = 3, seed_val = 781314)

# Compare 2009-2010 vs 2013-2014
do_year_comparison(X=cbind(bmd_men$Year_2007_2008,
                           bmd_men$Year_2009_2010+bmd_men$Year_2013_2014,
...
These analyses provide valid inference **conditional on passing the overall F-test**. The selective p-values and intervals are corrected for the selection step, avoiding overconfident inference that can arise from standard (naive) linear regression output.

Use this approach to interpret post-hoc comparisons more reliably in data exploration or confirmatory re-analysis.
