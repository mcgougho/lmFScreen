...
In this analysis we use the `lmFScreen` package to compare femur neck bone mineral density (BMD) across NHANES cycles for men over age 30. We first perform an overall F-test to test whether there is a change in BMD across all years. This test is rejected, so we conduct post-hoc tests for the individual coefficients for each year/cycle. We adjust for the fact that inference is conditioned on passing a model selection step — in our case, an overall F-test.
